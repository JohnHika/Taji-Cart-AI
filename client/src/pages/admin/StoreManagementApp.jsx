...
import {
  FaArrowRight, FaBoxes, FaBrain, FaChartLine, FaCheckCircle, FaClipboardList, FaPlus,
    ...
    <section className="space-y-5">
      <div className="relative overflow-hidden rounded-[30px] bg-[#260c28] px-5 py-6 text-white shadow-[0_28px_60px_-32px_rgba(40,8,41,0.9)] sm:px-7">
        ...
        <div className="relative flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
          <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-gold-300">Procurement desk</p><h2 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">Buy with visibility, receive with confidence.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-white/65">Every supplier commitment becomes a traceable stock movement—not another forgotten WhatsApp note.</p></div>
...
const StoreManagementWorkspace = () => {
  ...
  const areaContent = {
    ...
    purchasing: <section className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><div className={`${shell} p-4`}><p className={label}>Active suppliers</p><p className="mt-2 text-2xl font-black text-charcoal dark:text-white">{procurement.suppliers || 0}</p></div><div className={`${shell} p-4`}><p className={label}>Open purchase orders</p><p className="mt-2 text-2xl font-black text-charcoal dark:text-white">{procurement.openOrders || 0}</p></div><div className={`${shell} p-4`}><p className={label}>Units inbound</p><p className="mt-2 text-2xl font-black text-charcoal dark:text-white">{procurement.inboundUnits || 0}</p></div></div><section className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]"><div className={`${shell} p-5`}><p className={label}>Create records</p><h2 className="mt-1 text-lg font-black text-charcoal dark:text-white">Supplier and purchase order desk</h2><form onSubmit={createSupplier} className="mt-4 grid gap-2 sm:grid-cols-2"><input value={supplierDraft.name} onChange={(event) => setSupplierDraft((current) => ({ ...current, name: event.target.value }))} placeholder="Supplier name" className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none focus:border-plum-500 dark:border-dm-border dark:bg-dm-card-2"/><input value={supplierDraft.contactName} onChange={(event) => setSupplierDraft((current) => ({ ...current, contactName: event.target.value }))} placeholder="Contact person" className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none focus:border-plum-500 dark:border-dm-border dark:bg-dm-card-2"/><input value={supplierDraft.phone} onChange={(event) => setSupplierDraft((current) => ({ ...current, phone: event.target.value }))} placeholder="Phone" className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none focus:border-plum-500 dark:border-dm-border dark:bg-dm-card-2"/><button type="submit" disabled={procurementBusy} className="inline-flex items-center justify-center gap-2 rounded-xl border border-plum-200 px-3 py-2.5 text-sm font-bold text-plum-700 disabled:opacity-50"><FaPlus size={11}/> Add supplier</button></form><form onSubmit={createPurchaseOrder} className="mt-5 border-t border-brown-100 pt-5 dark:border-dm-border"><p className={label}>New one-line purchase order</p><div className="mt-2 grid gap-2 sm:grid-cols-2"><select value={purchaseDraft.supplierId} onChange={(event) => setPurchaseDraft((current) => ({ ...current, supplierId: event.target.value }))} className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none dark:border-dm-border dark:bg-dm-card-2"><option value="">Choose supplier</option>{(procurement.supplierList || []).map((supplier) => <option key={supplier._id} value={supplier._id}>{supplier.name}</option>)}</select><select value={purchaseDraft.productId} onChange={(event) => { const product = inventory.find((item) => item._id === event.target.value); setPurchaseDraft((current) => ({ ...current, productId: event.target.value, unitCost: product?.costPrice || current.unitCost })); }} className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none dark:border-dm-border dark:bg-dm-card-2"><option value="">Choose product</option>{inventory.map((product) => <option key={product._id} value={product._id}>{product.name}{product.sku ? ` · ${product.sku}` : ''}</option>)}</select><input type="number" min="1" value={purchaseDraft.quantity} onChange={(event) => setPurchaseDraft((current) => ({ ...current, quantity: event.target.value }))} placeholder="Quantity" className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none dark:border-dm-border dark:bg-dm-card-2"/><input type="number" min="0" value={purchaseDraft.unitCost} onChange={(event) => setPurchaseDraft((current) => ({ ...current, unitCost: event.target.value }))} placeholder="Cost per unit (KES)" className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none dark:border-dm-border dark:bg-dm-card-2"/><input type="date" value={purchaseDraft.expectedDate} onChange={(event) => setPurchaseDraft((current) => ({ ...current, expectedDate: event.target.value }))} className="rounded-xl border border-brown-200 bg-ivory px-3 py-2.5 text-sm outline-none dark:border-dm-border dark:bg-dm-card-2"/><button type="submit" disabled={procurementBusy} className="rounded-xl bg-plum-700 px-3 py-2.5 text-sm font-bold text-white disabled:opacity-50">Save draft PO</button></div></form></div><div className={`${shell} overflow-hidden`}><div className="border-b border-brown-100 p-5 dark:border-dm-border"><p className={label}>Supplier commitments</p><h2 className="mt-1 text-lg font-black text-charcoal dark:text-white">Purchase orders and receiving</h2></div><div className="divide-y divide-brown-100 dark:divide-dm-border">{(procurement.purchaseOrders || []).slice(0, 8).map((order) => <div key={order._id} className="p-4"><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="font-black text-charcoal dark:text-white">{order.number}</p><p className="mt-0.5 text-xs text-brown-500">{order.supplier?.name || 'Supplier'} · {order.lines.reduce((total, line) => total + line.orderedQuantity, 0)} units</p></div><span className="rounded-md bg-ivory px-2 py-1 text-xs font-bold capitalize text-brown-700 dark:bg-dm-card-2 dark:text-white/70">{order.status.replaceAll('_', ' ')}</span></div><p className="mt-2 text-xs text-brown-500">{order.lines.map((line) => `${line.productName}: ${line.receivedQuantity}/${line.orderedQuantity}`).join(' · ')}</p><div className="mt-3 flex flex-wrap gap-2">{order.status === 'draft' && <button type="button" disabled={procurementBusy} onClick={() => advancePurchaseOrder(order, 'order')} className="rounded-lg border border-plum-200 px-3 py-1.5 text-xs font-bold text-plum-700">Mark ordered</button>}{['ordered', 'partially_received'].includes(order.status) && <button type="button" disabled={procurementBusy} onClick={() => advancePurchaseOrder(order, 'receive')} className="rounded-lg bg-plum-700 px-3 py-1.5 text-xs font-bold text-white">Receive remainder</button>}</div></div>)}{!loading && !(procurement.purchaseOrders || []).length && <p className="p-8 text-center text-sm text-brown-500">Add a supplier, then create the first purchase order.</p>}</div></div></section></section>,
