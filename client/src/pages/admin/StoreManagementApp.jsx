...
import {
  ...
  FaExclamationTriangle, FaLock, FaSearch, FaSignOutAlt, FaStore,
...
const StoreManagementWorkspace = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [briefResponse, inventoryResponse] = await Promise.all([
      ...
      setInventory(inventoryResponse.data?.data?.products || []);
    } catch (error) {
  ...
  const ask = async (event, suggestedQuestion) => {
    ...
    try {
      const response = await Axios({
        ...
        data: { question: currentQuestion, range: 'today', sources: ['counter', 'online', 'inventory', 'delivery'], history },
      });
  ...
  const areaContent = {
    ...
    fulfillment: <section className="grid gap-4 lg:grid-cols-2"><div className={`${shell} p-5`}><p className={label}>Orders in motion</p><h2 className="mt-1 text-lg font-black text-charcoal dark:text-white">Fulfilment control</h2><div className="mt-5 grid grid-cols-2 gap-3"><div className="rounded-xl bg-ivory p-4 dark:bg-dm-card-2"><p className={label}>Open online orders</p><p className="mt-2 text-2xl font-black text-charcoal dark:text-white">{metrics.openOrderCount || 0}</p></div><div className="rounded-xl bg-ivory p-4 dark:bg-dm-card-2"><p className={label}>Active deliveries</p><p className="mt-2 text-2xl font-black text-charcoal dark:text-white">{metrics.activeDeliveryCount || 0}</p></div></div><div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => openMain('/dashboard/allorders')} className="rounded-xl border border-plum-200 px-4 py-2.5 text-sm font-bold text-plum-700">Manage orders</button><button type="button" onClick={() => openMain('/dashboard/staff/delivery')} className="rounded-xl border border-plum-200 px-4 py-2.5 text-sm font-bold text-plum-700">Manage delivery</button></div></div><div className={`${shell} p-5`}><p className={label}>Operating question</p><h2 className="mt-1 text-lg font-black text-charcoal dark:text-white">What could delay a customer today?</h2><p className="mt-3 text-sm leading-6 text-brown-500 dark:text-white/55">Ask the copilot to combine order volume, drivers, counter traffic, and stock risk before you assign work.</p><button type="button" onClick={() => setQuestionAndOpen('Are there any fulfilment risks today? Ask one follow-up question if a manager decision is needed.')} className="mt-5 rounded-xl bg-plum-700 px-4 py-2.5 text-sm font-bold text-white">Review fulfilment</button></div></section>,
    team: <section className="grid gap-4 lg:grid-cols-2"><div className={`${shell} p-5`}><p className={label}>People and permissions</p><h2 className="mt-1 text-lg font-black text-charcoal dark:text-white">Control who can do what</h2><p className="mt-3 text-sm leading-6 text-brown-500 dark:text-white/55">Staff roles, permissions, drivers, and customer accounts remain protected in the main admin system. This workspace keeps the operational context together and sends you to the exact control when a change is needed.</p><div className="mt-5 flex flex-wrap gap-2"><button type="button" onClick={() => openMain('/dashboard/users-admin')} className="rounded-xl bg-plum-700 px-4 py-2.5 text-sm font-bold text-white">Manage people</button><button type="button" onClick={() => openMain('/dashboard/driver-verification')} className="rounded-xl border border-plum-200 px-4 py-2.5 text-sm font-bold text-plum-700">Verify drivers</button></div></div><div className="rounded-2xl bg-charcoal p-5 text-white"><FaCheckCircle className="text-gold-300"/><h2 className="mt-3 text-lg font-black">Admin session only</h2><p className="mt-2 text-sm leading-6 text-white/65">Store Management uses a one-time hand-off from the primary Nawiri site and expires with this browser session. Knowing the hostname does not unlock it.</p><button type="button" onClick={leaveStorePortal} className="mt-5 inline-flex items-center gap-2 rounded-xl border border-white/20 px-4 py-2.5 text-sm font-bold hover:bg-white/10"><FaSignOutAlt size={12}/> Leave Store Management</button></div></section>,
