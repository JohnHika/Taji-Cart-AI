    ...
    <div className="min-h-screen w-full max-w-full overflow-x-hidden bg-gray-100 dark:bg-gray-900 p-4 pb-24 lg:pb-6">
            ...
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {recentSales.map(sale => (
                <tr key={sale._id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-500 dark:text-gray-300 max-w-[280px]">
                    {(sale.items && sale.items.length > 0)
                      ? (
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <button
                      onClick={() => { setSelectedSale(sale); setShowReceiptModal(true); }}
                      className="px-3 py-1.5 rounded-md border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300"
                    >
                      View Receipt
                    </button>
                  </td>
      ...
      {showReceiptModal && selectedSale && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-[440px] max-w-full mx-4">
            <div className="flex justify-between items-center mb-3">
              <h4 className="text-lg font-semibold text-gray-900 dark:text-white">Receipt #{selectedSale.saleNumber}</h4>
            ...
            <div className="border-t border-b border-gray-200 dark:border-gray-700 py-2 max-h-64 overflow-y-auto">
                ...
                <div className="mt-1 space-y-0.5">
                  {selectedSale.payments.map((p, i) => (
                    <div key={i} className="flex justify-between">
                      <span className="text-gray-500 dark:text-gray-400">{p.method === 'mobile' ? `M-Pesa${p.phone ? ` (${p.phone})` : ''}` : p.method}</span>
            ...
            {selectedSale.note && (
              <div className="mt-3 text-xs text-gray-500 italic">Note: {selectedSale.note}</div>
            )}
            <div className="mt-4 flex justify-end">
              <button onClick={() => setShowReceiptModal(false)} className="px-4 py-2 rounded-lg border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 text-sm text-gray-700 dark:text-gray-300">Close</button>
            </div>
          </div>
        </div>
      )}
