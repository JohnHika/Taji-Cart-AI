...
const UploadDeliveryZoneModal = ({ close, fetchData }) => {
    const [data, setData] = useState({
        name: "",
        corridor: "",
        fare: "",
        inNairobiCounty: false
    })
    ...
    const isValid = data.name.trim() && data.corridor.trim() && Number(data.fare) > 0
        ...
        <section className='fixed top-0 bottom-0 left-0 right-0 p-4 bg-neutral-800 bg-opacity-60 z-50 flex items-center justify-center'>
            <div className='bg-white dark:bg-dm-card max-w-md w-full p-4 rounded transition-colors duration-200'>
                <div className='flex items-center justify-between'>
                    <h1 className='font-semibold text-charcoal dark:text-white transition-colors duration-200'>Add Delivery Zone</h1>
                ...
                <form className='my-3 grid gap-3' onSubmit={handleSubmit}>
                    ...
                    <button
                        ...
                        font-semibold
                        `}
                    >{loading ? 'Adding...' : 'Add Zone'}</button>
                </form>
            </div>
        </section>
    )
...
export default UploadDeliveryZoneModal
