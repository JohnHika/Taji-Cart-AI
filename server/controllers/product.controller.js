...
const PRODUCT_CODE_COLLATION = { locale: 'en', strength: 2 };
...
const ensureUniqueProductField = async ({ field, value, excludeId }) => {
    ...
    return ProductModel.findOne(filter).collation(PRODUCT_CODE_COLLATION);
};
...
export const createProductController = async(request,response)=>{
    try {
        ...
        if (normalizedBarcode && !isCode128BarcodeValue(normalizedBarcode)) {
            return response.status(400).json({
                message: 'Barcode can only use printable Code 128 characters.',
                error: true,
                success: false
            });
        }

        if (normalizedBarcode && isNawiriBarcodeNamespace(normalizedBarcode)) {
            return response.status(400).json({
                message: 'NWH barcodes are generated by Nawiri Hair. Leave the barcode field blank or enter the supplier barcode printed on the package.',
                error: true,
                success: false
            });
        }
        ...
        if(existingSKU) {
            return response.status(400).json({
                message : "SKU already exists. Use a different internal stock code.",
                error : true,
                success : false
            })
        }
...
export const updateProductDetails = async(request,response)=>{
    try {
        ...
        if (normalizedBarcode && isNawiriBarcodeNamespace(normalizedBarcode)) {
            const currentProduct = await ProductModel.findById(productIdToUse).select('barcode').lean();
            if (!currentProduct) {
                return response.status(404).json({
                    message: 'Product not found',
                    error: true,
            ...
            if (normalizeScanValue(currentProduct.barcode).toUpperCase() !== normalizedBarcode.toUpperCase()) {
                return response.status(400).json({
                    message: 'NWH barcodes are generated by Nawiri Hair and cannot be manually reassigned.',
                    error: true,
                    success: false
                });
            }
        ...
        if ('barcode' in request.body) {
            if (normalizedBarcode) {
                ...
                // An empty editor field must never erase a scan reference. Keep the
                // existing code, or repair a legacy product that has no barcode yet.
                delete updatePayload.barcode;
                const existingProduct = await ProductModel.findById(productIdToUse).select('barcode').lean();
                if (!existingProduct) {
                    return response.status(404).json({
                        message: 'Product not found',
                        error: true,
