...
export async function CashOnDeliveryOrderController(request, response) {
    try {
        ...
        // Orders dispatched to a SACCO/coach terminal for onward matatu
        // travel outside Nairobi carry more non-payment risk than a rider we
        // control delivering within Nairobi — those must be paid upfront via
        // M-Pesa/Card. Enforced here (not just hidden client-side) since this
        // endpoint could otherwise still be called directly with fulfillment_type
        // 'sacco_pickup'.
        if (fulfillment_type === 'sacco_pickup') {
            return response.status(400).json({
                message: "Orders outside Nairobi (SACCO/coach terminal drop-off) require payment upfront via M-Pesa or Card.",
                error: true,
                success: false,
                code: 'COD_NOT_ALLOWED_OUTSIDE_NAIROBI',
            });
        }
